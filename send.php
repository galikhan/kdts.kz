...
if ((isset($_SERVER['HTTP_X_REQUESTED_WITH'])) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
    ...
    if (isset($_GET['do']) && $_GET["do"] == 'cargo2') {
        $json = json_decode(connect('https://crm.kdts.kz/api/Site/search/GNGs?type=name&text=' . urlencode($_GET["q"])),1);
...
if ((isset($_SERVER['HTTP_X_REQUESTED_WITH'])) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
    ...
    if (isset($_POST['do']) && ($_POST['do'] == 'phone')) {
        ...
        echo json_encode(
            array(
                'success' => $success,
                'error' => $error,
                'msg' => '<p align="center" style="color:#fff;">Сіздің өтінішіңіз қабылданды, қоңырауды күтіңіз</p>',
            )
        );
    }
    ...
    if (isset($_POST['do']) && ($_POST['do'] == 'order')) {
        ...
        $type = [
            'Жіберу' => 'EXPEDITING',
            'Кешенді мөлшерлеме' => 'COMPLEX_RATE',
            'Пайдалану' => 'OPERATING'
        ];
        ...
        $sended = [
            'Бойдақ' => 'SINGLE',
            'Топ' => 'GROUP',
            'Контейнерлік пойыз' => 'CONTAINER_TRAIN'
        ];
        ...
        if ($send){
            ...
            echo json_encode(
                array(
                    'success' => $success,
                    'error' => $error,
                    'msg' => '<p align="center" style="color:#fff;">Сәлеметсіз бе, құрметті клиент!</p><p>&nbsp;</p><p align="center" style="color:#fff;">Хабарласқаныңыз үшін алғыс айтамыз. Сіздің өтінішіңіз табысты түрде қабылданды. Көрсетілген электрондық поштаға біздің коммерциялық ұсынысымыз жіберіледі. Егер қосымша сұрағыңыз болса, Call-center телефонына +7 717 2 94 26 26 хабарласыңыз.</p>',
                )
            );
        }else{
...
if (isset($_GET["test"])){
    ...
    $data = [
        ...
        'containerWeightCode' => $weight[$_GET["typec"]],
    ...
    //$json = json_decode(connect('https://crm.kdts.kz/api/Site/dicts'),1);
